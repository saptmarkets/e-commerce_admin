...
const ProductDrawer = ({ id }) => {
  ...
  // Function to update default ProductUnit price when Basic Info price changes
  const updateDefaultProductUnitPrice = async (newPrice) => {
    try {
      if (!id || !productUnits || productUnits.length === 0) {
        console.log('❌ Cannot update price: No product units available');
        return;
      }
      
      // Find the default ProductUnit
      const defaultProductUnit = productUnits.find(u => u.isDefault) || productUnits[0];
      if (!defaultProductUnit) {
        console.log('❌ Cannot update price: No default product unit found');
        return;
      }
      
      console.log(`🔄 Updating default ProductUnit price: ${defaultProductUnit.price} → ${newPrice}`);
      console.log(`📝 ProductUnit ID: ${defaultProductUnit._id}, SKU: ${defaultProductUnit.sku}`);
      
      // Update the ProductUnit price
      const updateResult = await ProductUnitServices.updateProductUnit(id, defaultProductUnit._id, {
        price: newPrice
      });
      
      if (updateResult && updateResult.success) {
        console.log(`✅ Default ProductUnit price updated to: ${newPrice}`);
        
        // Update local state to reflect the change
        setProductUnits(prev => prev.map(u => 
          u._id === defaultProductUnit._id 
            ? { ...u, price: newPrice }
            : u
        ));
        
        // Also update the form value to ensure consistency
        setValue("price", newPrice);
        
        notifySuccess(`Default unit price updated to ${newPrice}`);
      } else {
        console.error('❌ ProductUnit update failed:', updateResult);
        notifyError('Failed to update default unit price');
      }
    } catch (error) {
      console.error('❌ Error updating default ProductUnit price:', error);
      notifyError('Failed to update default unit price');
    }
  };
      ...
      const fetchProduct = async () => {
        ...
        try {
          ...
          if (productResult) {
            ...
            // Don't set price yet - we'll set it from the default ProductUnit
            ...
            if (unitsResult && unitsResult.data) {
              ...
              // If no global units were loaded, derive selectable units from product units
              if (!units || units.length === 0) {
                const derivedUnits = fetchedUnits
                  .map(u => u.unit)
                  .filter(Boolean)
                  .filter((v, i, a) => a.findIndex(x => (x?._id || x) === (v?._id || v)) === i);
                if (derivedUnits.length > 0) {
                  setUnits(derivedUnits);
                }
              }

              // Auto-select the product's default unit in Basic Info
              const defaultPU = fetchedUnits.find(u => u.isDefault) || fetchedUnits[0];
              if (defaultPU && (defaultPU.unit?._id || defaultPU.unit)) {
                setValue("basicUnit", defaultPU.unit?._id || defaultPU.unit);
                
                // Set the price from the default ProductUnit (not from Product.price)
                if (defaultPU.price !== undefined) {
                  setValue("price", defaultPU.price);
                  console.log(`💰 Setting price from default ProductUnit: ${defaultPU.price}`);
                }
              }
            }
          }
                  ...
                  <div className="col-span-8 sm:col-span-4">
                    <Input
                      {...register("price", {
                        ...
                        onChange: (e) => {
                          // When price changes, also update the default ProductUnit
                          const newPrice = parseFloat(e.target.value) || 0;
                          updateDefaultProductUnitPrice(newPrice);
                        }
                      })}
                      className="border h-8 text-sm focus:outline-none block w-full bg-gray-100 dark:bg-white border-transparent focus:bg-white"
                      name="price"
                    ...
                    <p className="text-xs text-blue-500 mt-1">
                      ℹ️ This price will update the default unit price in ProductUnits collection
                    </p>
                  </div>
                </div>
