...
const PromotionManagement = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // 🔥 FIXED: Use proper pagination instead of hardcoded limit
        const actualPageSize = showAllPages ? Math.min(totalPromotions || 1000, 1000) : pageSize;
        ...
        // 🔥 NEW: Update pagination info
        if (promotionsResponse?.totalPages) {
          setTotalPages(promotionsResponse.totalPages);
        }
        if (promotionsResponse?.totalPromotions) {
          setTotalPromotions(promotionsResponse.totalPromotions);
        }
  ...
  }, [listId, currentPage, pageSize, showAllPages, totalPromotions]);
  ...
  const handlePageChange = (newPage) => {
    ...
    setSelectedPromotions(new Set()); // Clear selection when changing pages
  };

  const handlePageSizeChange = (newPageSize) => {
    setPageSize(newPageSize);
    setCurrentPage(1); // Reset to first page when changing page size
    setSelectedPromotions(new Set()); // Clear selection
  };
  ...
  const confirmImport = async () => {
    ...
    try {
      ...
      const importPromises = validPromotions.map(item => {
        ...
        // 🔥 FIXED: Refresh promotions with proper pagination
        const actualPageSize = showAllPages ? Math.min(totalPromotions || 1000, 1000) : pageSize;
        const promotionsResponse = await PromotionServices.getAllPromotions({ 
          promotionList: listId,
          page: showAllPages ? 1 : currentPage,
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
              <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
                ...
                <div className="text-sm text-gray-600">
                  {showAllPages ? (
                    <span>Showing all {totalPromotions} promotions</span>
                  ) : (
                    <span>
                      Page {currentPage} of {totalPages} • {totalPromotions} total promotions
                    </span>
                  )}
                </div>
              </div>
