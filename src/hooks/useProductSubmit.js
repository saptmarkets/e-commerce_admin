  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (resData) {
        ...
        // 🔄 PRICE SYNCHRONIZATION: Ensure Basic Info price updates default ProductUnit
        if (data.price !== undefined && productUnits && productUnits.length > 0) {
          const defaultProductUnit = productUnits.find(u => u.isDefault) || productUnits[0];
          if (defaultProductUnit && defaultProductUnit.price !== parseFloat(data.price)) {
            console.log(`🔄 Syncing Basic Info price (${data.price}) with default ProductUnit (${defaultProductUnit.price})`);
            
            try {
              // Update the default ProductUnit price to match Basic Info price
              await ProductUnitServices.updateProductUnit(updatedId, defaultProductUnit._id, {
              ...
              setProductUnits(prev => prev.map(u => 
                u._id === defaultProductUnit._id 
                  ? { ...u, price: parseFloat(data.price) }
                  : u
              ));
              
              console.log(`✅ Default ProductUnit price synced to: ${data.price}`);
            } catch (syncError) {
              console.error('❌ Failed to sync ProductUnit price:', syncError);
              notifyError('Price updated but failed to sync with default unit');
            }
          }
        }
